...
class RuntimeLLMTests(unittest.TestCase):
    def test_ignore_keeps_event_unstopped_and_does_not_call_llm(self) -> None:
        context = FakeContext([])
        runtime = QianjiLingqueRuntime(context, PluginConfig())
    ...
    def test_prepare_agent_run_disables_tools_for_passive_reply(self) -> None:
        context = FakeContext([])
        runtime = QianjiLingqueRuntime(context, PluginConfig(bot_aliases=["机器人"]))
        event = FakeGroupEvent("机器人，帮我看看这个怎么弄？")
        ...
        self.assertNotIn("Current workspace you can use", request.system_prompt)
        self.assertNotIn("When using tools:", system_message.content)
        self.assertNotIn("You MUST NOT return an empty response", system_message.content)
        self.assertNotIn("host local environment", system_message.content)
        self.assertNotIn("Current workspace you can use", system_message.content)
    ...
    def test_starting_pending_blocks_parallel_non_direct_builds(self) -> None:
        async def scenario() -> tuple[list[object], list[object]]:
            ...
            config = PluginConfig(bot_aliases=["机器人"], score_threshold_reply=0.1, score_threshold_ignore=0.0)
    ...
    def test_pending_non_direct_reply_blocks_direct_request_with_busy_hint(self) -> None:
        context = FakeContext([])
        config = PluginConfig(bot_aliases=["机器人"], score_threshold_reply=0.1, score_threshold_ignore=0.0)
        runtime = QianjiLingqueRuntime(context, config)
    ...
    def test_takeover_explicit_mentions_can_be_disabled(self) -> None:
        ...
        runtime = QianjiLingqueRuntime(
            context,
            PluginConfig(bot_aliases=["机器人"], takeover_explicit_mentions=False),
        )
        event = FakeGroupEvent("@bot 帮我看看这个怎么弄？", at_bot=True)
    ...
    def test_wake_prefix_reply_suppresses_default_without_stopping_event(self) -> None:
        context = FakeContext([])
        runtime = QianjiLingqueRuntime(context, PluginConfig(bot_aliases=["机器人"]))
    ...
    def test_previous_send_flag_does_not_confirm_plugin_reply_without_current_result(self) -> None:
        context = FakeContext([])
        runtime = QianjiLingqueRuntime(context, PluginConfig(bot_aliases=["机器人"]))
        event = FakeGroupEvent("机器人，帮我看看这个怎么弄？")
        event._has_send_oper = True
        ...
        self.assertIn("早于本插件请求", state.last_decision)
    ...
    def test_late_response_after_timeout_still_records_sent_reply(self) -> None:
        context = FakeContext([])
        runtime = QianjiLingqueRuntime(context, PluginConfig(bot_aliases=["机器人"], reply_timeout_seconds=1.0))
        event = FakeGroupEvent("机器人，帮我看看这个怎么弄？")
    ...
    def test_unconfirmed_response_expires_quickly_without_after_hook(self) -> None:
        context = FakeContext([])
        runtime = QianjiLingqueRuntime(context, PluginConfig(bot_aliases=["机器人"], reply_timeout_seconds=10.0))
        event = FakeGroupEvent("机器人，帮我看看这个怎么弄？")
        event.extra["enable_streaming"] = True
    ...
    def test_platform_instance_isolates_pending_and_context(self) -> None:
        ...
        self.assertIsNotNone(runtime.context_store.peek_group("onebot-b:group-1"))
    ...
    def test_platform_instance_mode_uses_isolated_config_key(self) -> None:
        ...
        self.assertEqual(config.group_modes["onebot-a:group-1"], "active")

    def test_platform_instance_accepts_legacy_bare_enabled_group(self) -> None:
        context = FakeContext([])
        config = PluginConfig(enabled_groups=["group-1"], bot_aliases=["机器人"])
        runtime = QianjiLingqueRuntime(context, config)
        event = FakeGroupEvent(
            "机器人，帮我看看？",
            platform_id="onebot-a",
    ...
    def test_platform_instance_accepts_legacy_bare_group_mode(self) -> None:
        context = FakeContext([])
        config = PluginConfig(group_modes={"group-1": "active"}, bot_aliases=["机器人"])
        runtime = QianjiLingqueRuntime(context, config)
        event = FakeGroupEvent(
            "这个怎么办？帮忙看看",
            platform_id="onebot-a",
